...
const defaultRoutesList: TaxiRoute[] = [
...
function getRandomRidesCount(from: string, to: string): string {
  ...
  const count = Math.abs(hash % 1800) + 200; // between 200 and 2000
  return `${count}+`;
}

function getRouteCombinations(searchText: string): TaxiRoute[] {
  const query = searchText.trim().toLowerCase();
  if (!query) {
    return defaultRoutesList;
  }

  const findMatchingPlaces = (q: string) => 
    biharPlaces.filter(p => p.toLowerCase().includes(q));

  const popularHubs = [
    'Patna', 'Gaya', 'Muzaffarpur', 'Darbhanga', 'Bodh Gaya', 
    'Rajgir', 'Nalanda', 'Patna Airport', 'Darbhanga Airport', 
    'Deoghar', 'Danapur', 'Patna Junction'
  ];

  let origins: string[] = [];
  let destinations: string[] = [];

  // Case 1: Query contains " to "
  if (query.includes(' to ')) {
    ...
    if (originPart) {
      origins = findMatchingPlaces(originPart);
    }
    if (destPart) {
      destinations = findMatchingPlaces(destPart);
    }

    // Fill empty side with popular hubs
    if (origins.length > 0 && destinations.length === 0) {
      destinations = popularHubs.filter(p => !origins.some(o => o.toLowerCase() === p.toLowerCase()));
    } else if (destinations.length > 0 && origins.length === 0) {
      origins = popularHubs.filter(p => !destinations.some(d => d.toLowerCase() === p.toLowerCase()));
    }
  } 
  // Case 2: No " to " in query, try splitting by space to see if it's two separate places
  else {
    ...
    for (let i = 1; i < words.length; i++) {
      ...
      const matched1 = findMatchingPlaces(part1);
      const matched2 = findMatchingPlaces(part2);

      if (matched1.length > 0 && matched2.length > 0) {
        origins = matched1;
        destinations = matched2;
        splitFound = true;
        break;
      }
    }

    // Case 3: Single place (or single prefix matching multiple places)
    if (!splitFound) {
      const matchedPlaces = findMatchingPlaces(query);
      if (matchedPlaces.length > 0) {
        const routes: TaxiRoute[] = [];
        
        matchedPlaces.forEach(place => {
          popularHubs.forEach(hub => {
            if (place.toLowerCase() !== hub.toLowerCase()) {
              routes.push({
                route: `${place} to ${hub}`,
                category: 'Bihar Taxi Route',
                ridesCount: getRandomRidesCount(place, hub)
              });
            }
          });
          popularHubs.forEach(hub => {
            if (hub.toLowerCase() !== place.toLowerCase()) {
              const routeStr = `${hub} to ${place}`;
              if (!routes.some(r => r.route.toLowerCase() === routeStr.toLowerCase())) {
                routes.push({
                  route: routeStr,
                  category: 'Bihar Taxi Route',
                  ridesCount: getRandomRidesCount(hub, place)
                });
              }
            }
          });
  ...
  // Generate combinations from origins and destinations
  const combinations: TaxiRoute[] = [];
  if (origins.length > 0 && destinations.length > 0) {
    origins.forEach(org => {
      destinations.forEach(dest => {
        if (org.toLowerCase() !== dest.toLowerCase()) {
          combinations.push({
            route: `${org} to ${dest}`,
            category: 'Bihar Taxi Route',
            ridesCount: getRandomRidesCount(org, dest)
          });
        }
      });
    });
...
interface DestinationDropdownProps {
  ...
  onSelectRoute: (route: TaxiRoute) => void;
  placeholder?: string;
  className?: string;
}

function DestinationDropdown({ selectedRoute, onSelectRoute, placeholder = "Where are you going?", className = "" }: DestinationDropdownProps) {
  ...
  const routes = React.useMemo(() => {
    return getRouteCombinations(searchText);
  }, [searchText]);
    ...
    <div className="single-search-box" ref={dropdownRef}>
      ...
      <div className={`custom-select-wrap ${className} ${isOpen ? 'active' : ''}`} style={{ display: 'block', opacity: isOpen ? 1 : 0, transform: isOpen ? 'scaleY(1)' : 'scaleY(0)', transformOrigin: 'top', transition: 'transform 0.15s ease-out, opacity 0.15s ease-out', pointerEvents: isOpen ? 'auto' : 'none', zIndex: 9999 }}>
        <div className="custom-select-search-area">
          ...
          <input 
            type="text" 
            placeholder="Type Your Destination Route" 
            value={searchText}
        ...
        <ul className="option-list-destination">
          {routes.length > 0 ? (
            routes.map((r, i) => (
              <li key={i} onClick={() => { onSelectRoute(r); setIsOpen(false); }}>
                <div className="destination">
                  <h6>{r.route}</h6>
                  <span>{r.category}</span>
                </div>
                <div className="tour">
                  <span>{r.ridesCount.split(' ')[0]} <br /> Rides</span>
                </div>
              </li>
            ))
          ) : (
            <li className="no-result" style={{ padding: '10px 15px', color: '#888', pointerEvents: 'none' }}>
              No routes found
            </li>
          )}
        </ul>
...
export default function HomeFilter() {
  const [oneWayRoute, setOneWayRoute] = React.useState<TaxiRoute>({ route: 'Patna to Darbhanga', category: 'Bihar Taxi Route', ridesCount: '2000+' });
  const [roundTripRoute, setRoundTripRoute] = React.useState<TaxiRoute>({ route: 'Patna to Darbhanga', category: 'Bihar Taxi Route', ridesCount: '2000+' });
  const [outstationRoute, setOutstationRoute] = React.useState<TaxiRoute>({ route: 'Patna to Darbhanga', category: 'Bihar Taxi Route', ridesCount: '2000+' });
  
  const [airportRoute, setAirportRoute] = React.useState<TaxiRoute>({ route: 'Patna Airport (PAT)', category: 'Patna, Bihar', ridesCount: '24x7 Service' });
  const [airportIsOpen, setAirportIsOpen] = React.useState(false);
  ...
  const airports = [
    { route: 'Patna Airport (PAT)', category: 'Patna, Bihar', ridesCount: '24x7 Service' },
    { route: 'Darbhanga Airport (DBG)', category: 'Darbhanga, Bihar', ridesCount: '24x7 Service' },
    { route: 'Gaya Airport (GAY)', category: 'Gaya, Bihar', ridesCount: '24x7 Service' }
  ];

  const filteredAirports = React.useMemo(() => {
    return airports.filter(a => a.route.toLowerCase().includes(airportSearch.toLowerCase()) || a.category.toLowerCase().includes(airportSearch.toLowerCase()));
  }, [airportSearch]);
  ...
  const handleSubmitOneWay = (e: React.FormEvent) => {
    ...
    const dateElement = form.querySelector('.selected-date h6');
    const dateText = dateElement ? dateElement.textContent : '';
    const yearElement = form.querySelector('.selected-date span');
    const yearText = yearElement ? yearElement.textContent : '';
    const dateStr = dateText ? `${dateText} ${yearText}` : 'Select Date & Time';

    const carInput = form.querySelector('input[defaultValue="Select Car Type"]') as HTMLInputElement;
    const carType = carInput ? carInput.value : 'Dzire/Etios';
  ...
  const handleSubmitRoundTrip = (e: React.FormEvent) => {
    ...
    const checkinElement = form.querySelector('.hotel-selected-date-checkin h6');
    const checkinText = checkinElement ? checkinElement.textContent : '';
    const checkoutElement = form.querySelector('.hotel-selected-date-checkout h6');
    const checkoutText = checkoutElement ? checkoutElement.textContent : '';
    const dateStr = (checkinText && checkoutText) ? `${checkinText} to ${checkoutText}` : 'June 16 - June 20';
  ...
  const handleSubmitOutstation = (e: React.FormEvent) => {
    e.preventDefault();
    const form = e.currentTarget as HTMLFormElement;
  ...
  const handleSubmitAirport = (e: React.FormEvent) => {
    ...
    const transferInput = form.querySelector('input[defaultValue="Select Transfer Type"]') as HTMLInputElement;
    const transferType = transferInput ? transferInput.value : 'Pickup from Airport';

    const durationInput = form.querySelector('input[defaultValue="1 Day"]') as HTMLInputElement;
    const duration = durationInput ? durationInput.value : '1 Day';
    ...
    <div className="filter-wrapper">
      <div className="container">
        ...
        <div className="filter-input-wrap">
          {/* ONE-WAY TAXI FORM */}
          <form className="filter-input show" onSubmit={handleSubmitOneWay}>
            <DestinationDropdown 
              selectedRoute={oneWayRoute}
              onSelectRoute={setOneWayRoute}
            ...
            <div className="single-search-box date-field">
              ...
              <div className="custom-select-dropdown">
                <input type="text" name="inOut" readOnly defaultValue="Select Date & Time" />
            ...
            <div className="single-search-box">
              <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <g>
                  ...
                  <path d="M3.48826 8.83719C5.41485 8.83719 6.97665 7.27538 6.97665 5.3488C6.97665 3.42222 5.41485 1.86041 3.48826 1.86041C1.56168 1.86041 -0.00012207 3.42222 -0.00012207 5.3488C-0.00012207 7.27538 1.56168 8.83719 3.48826 8.83719Z" />
                  <path d="M3.48826 18.1396C5.41485 18.1396 6.97665 16.5778 6.97665 14.6512C6.97665 12.7246 5.41485 11.1628 3.48826 11.1628C1.56168 11.1628 -0.00012207 12.7246 -0.00012207 14.6512C-0.00012207 16.5778 1.56168 18.1396 3.48826 18.1396Z" />
                </g>
              </svg>
              <div className="custom-select-dropdown">
                <input type="text" readOnly defaultValue="Select Car Type" />
                <span>Category</span>
              </div>
          ...
          <form className="filter-input two" onSubmit={handleSubmitRoundTrip}>
            <DestinationDropdown 
              selectedRoute={roundTripRoute}
              onSelectRoute={setRoundTripRoute}
              placeholder="Where are you going?"
              className="three"
            />
            ...
            <div className="single-search-box date-field">
              <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <g>
                  <path d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72272 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.55875H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z" />
                  <path d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z" />
                  <path d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 3.15172 11.2512 2.84106V0.591064Z" />
                  <path d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z" />
                  <path d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z" />
          ...
          {/* OUTSTATION TAXI FORM */}
          <form className="filter-input two" onSubmit={handleSubmitOutstation}>
            <DestinationDropdown 
              selectedRoute={outstationRoute}
              onSelectRoute={setOutstationRoute}
              placeholder="Where are you going?"
              className="four"
            />
            ...
            <div className="single-search-box date-field">
              <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <g>
                  <path d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72272 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.58754C6.75202 3.5165 6.64033 3.41386 6.55848 3.28887C6.47662 3.16388 6.42716 3.02047 6.41457 2.87159C6.40199 2.72272 6.42666 2.57304 6.48638 2.43609V1.28394H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z" />
                  <path d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z" />
                  <path d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 2.84106V0.591064Z" />
                  <path d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z" />
                  <path d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z" />
                  <path d="M5.57379 12.859C5.57379 11.841 6.19393 11.266 6.94745 10.9237C6.31772 10.5738 5.93327 9.97518 5.93327 9.23362C5.93327 7.84346 7.14253 6.93768 9.03335 6.93768C10.665 6.93768 12.0754 7.71146 12.0754 9.2562C12.0754 10.0578 11.5991 10.5852 11.0117 10.8392C11.8151 11.133 12.4262 11.8054 12.4262 12.8442C12.4262 14.553 10.7024 15.3177 8.95704 15.3177C7.14785 15.3177 5.57379 14.5132 5.57379 12.859ZM10.4611 12.8062C10.4611 12.1583 10.0752 11.6429 8.99162 11.6429C7.89793 11.6429 7.50868 12.1281 7.50868 12.7625C7.50868 13.578 8.28429 13.9316 8.9993 13.9316C9.72377 13.9316 10.4611 13.636 10.4611 12.8062ZM7.83377 9.24273C7.83377 9.7755 8.13992 10.2237 9.04127 10.2237C9.88592 10.2237 10.171 9.82871 10.171 9.25623C10.171 8.62605 9.6497 8.29207 8.99612 8.29207C8.39034 8.29203 7.83377 8.57565 7.83377 9.24273Z" />
                </g>
          ...
          <form className="filter-input" onSubmit={handleSubmitAirport}>
            <div className="single-search-box" ref={airportsRef}>
              <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <g>
                  <path d="M12.5944 8.99987C12.5944 10.988 10.9826 12.5998 8.99443 12.5998C7.00627 12.5998 5.39465 10.988 5.39465 8.99987C5.39465 7.0117 7.00627 5.40009 8.99443 5.40009C10.9826 5.40009 12.5944 7.0117 12.5944 8.99987Z" />
                  <path d="M17.4601 8.4599H16.2564C15.9858 4.86535 13.1291 2.00812 9.53458 1.7372V0.539976C9.53458 0.241723 9.29268 0 8.9946 0C8.69635 0 8.45462 0.241723 8.45462 0.539976V1.7372C4.85986 2.00812 2.00297 4.86535 1.73235 8.4599H0.540018C0.241723 8.4599 0 8.7017 0 8.99987C0 9.29813 0.241723 9.53985 0.539976 9.53985H1.73239C2.00297 13.1344 4.85991 15.9916 8.45441 16.2625V17.4601C8.45441 17.7583 8.69614 18 8.99439 18C9.29251 18 9.53428 17.7583 9.53428 17.4601V16.2625C13.1289 15.9918 15.9858 13.1346 16.2564 9.53985H17.4601C17.7583 9.53985 18 9.29813 18 8.99987C18 8.70175 17.7583 8.4599 17.4601 8.4599ZM8.99443 15.2096C5.56504 15.2094 2.78509 12.4291 2.78509 8.9997C2.78522 5.57014 5.56554 2.7902 8.99494 2.7902C12.4245 2.7902 15.2046 5.57048 15.2046 8.99987C15.2005 12.428 12.4225 15.2058 8.99443 15.2096Z" />
              ...
              <div className="custom-select-dropdown destination-dropdown" onClick={() => setAirportIsOpen(!airportIsOpen)}>
                ...
                <div className="input-field-value">
                  <div className="destination">
                    <h6>{airportRoute.route}</h6>
                    <span>{airportRoute.category}</span>
                  </div>
                </div>
              </div>
              <div className={`custom-select-wrap five ${airportIsOpen ? 'active' : ''}`} style={{ display: airportIsOpen ? 'block' : 'none', transform: airportIsOpen ? 'scaleY(1)' : 'scaleY(0)', transformOrigin: 'top', transition: 'transform 0.15s ease-out' }}>
                <div className="custom-select-search-area">
                  ...
                  <input 
                    ...
                    autoFocus={airportIsOpen}
                  />
                </div>
                <ul className="option-list-destination">
                  {filteredAirports.map((a, i) => (
                    <li key={i} onClick={() => { setAirportRoute(a); setAirportIsOpen(false); }}>
                      <div className="destination">
                        <h6>{a.route}</h6>
                        <span>{a.category}</span>
                      </div>
                      <div className="tour">
                        <span>24x7 <br /> Service</span>
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <div className="single-search-box">
              ...
              <div className="custom-select-dropdown">
                <input type="text" readOnly defaultValue="Select Transfer Type" />
                <span>Transfer Type</span>
              </div>
              <div className="custom-select-wrap two">
                <ul className="option-list">
                  <li className="single-item">
                    <h6>Pickup from Airport</h6>
                  </li>
                  <li className="single-item">
                    <h6>Drop to Airport</h6>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="single-search-box date-field">
              <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <g>
                  <path d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72275 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.58754C6.75202 3.5165 6.64033 3.41386 6.55848 3.28887C6.47662 3.16388 6.42716 3.02047 6.41457 2.87159C6.40199 2.72272 6.42666 2.57304 6.48638 2.43609V1.28394H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z" />
                  <path d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z" />
                  <path d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 2.84106V0.591064Z" />
                  <path d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z" />
                  <path d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z" />
            ...
            <div className="single-search-box">
              ...
              <div className="custom-select-dropdown">
                <input type="text" readOnly defaultValue="1 Day" />
                <span>Duration</span>
              </div>
              <div className="custom-select-wrap two">
                <ul className="option-list">
                  <li className="single-item"><h6>1 Day</h6></li>
                  <li className="single-item"><h6>2 Days</h6></li>
                  <li className="single-item"><h6>3 Days</h6></li>
                  <li className="single-item"><h6>4 Days</h6></li>
                  <li className="single-item"><h6>5+ Days</h6></li>
                </ul>
              </div>
            </div>
